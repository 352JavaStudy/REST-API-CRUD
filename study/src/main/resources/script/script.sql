...
create table free_board(
    ...
    constraints fk_free_user_id foreign key(free_user_id) references USER_TBL(user_id) on delete cascade
);
...
create table travel_board(
    ...
    constraints fk_travel_user_id foreign key(travel_user_id) references user_tbl(user_id) on delete cascade
);
...
create table image_attachment (
    ...
    constraint fk_free_id foreign key(free_id) references free_board(free_id),
    constraint fk_food_id foreign key(food_id) references food_board(food_id),
    constraint fk_travel_id foreign key(travel_id) references travel_board(travel_id)
);
